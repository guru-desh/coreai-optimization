...
class TestKmeans1D:
    ...
    def test_collapse_fewer_than_k_clusters(
        ...
        # tie-breaking mechanics get run. This branch's Hirschberg-style
        # divide-and-conquer backtracking spreads ties evenly across clusters
        # rather than the original sequential backward-recursion's convention of
        # collapsing everything into the last cluster -- both are equally optimal,
        # so only the reconstruction (which is unaffected by exactly this
        # tie-breaking choice) is checked here, not the exact centroids array.
